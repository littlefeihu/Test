// 源文件头信息：
// <copyright file="WebHttpClient.cs">
...
namespace ExpressionSerialization
{
    public class WebHttpClient<TChannel> where TChannel : IQueryService
    {
        ...
        {
            get { return _knownTypes.AsEnumerable(); }
        }
        ...
        {
            return (TResult)SynchronousCall(methodcall: methodcall, returnType: typeof(TResult));
        }

        /// <summary>
        ///   Specifying the returnType is necessary because sometimes DataContractSerializer cannot deserialize the response without knowing the exact Type of the response. Sometimes DCS fails to deserialize, sometimes it doesn't, but the best practice is to always specify the expected return Type. If calling from SL, this call must be made from a thread separate from the main UI thread. For example, enclose this call within a call to ThreadPool.QueueUserWorkItem.
        /// </summary>
        ...
        public object SynchronousCall(Expression<Func<TChannel, object>> methodcall, Type returnType)
        {
            ...
            CreateHttpWebRequest(endpoint, instance: parameters, callback: completedHandler, method: method, requestFormat: requestformat,
        ...
        private void CreateHttpWebRequest(Uri absoluteUri, object instance, Action<Stream> callback, string method = "POST",
            WebMessageFormat requestFormat = WebMessageFormat.Xml, WebMessageFormat responseFormat = WebMessageFormat.Xml)
        {
            Stream postStream;
        ...
        private static Uri GetOperationInfo(MethodInfo operation, Uri baseAddress, out string method, out IOperationBehavior webbehavior,
            //out WebInvokeAttribute webinvoke,
            out OperationContractAttribute operationcontract, out WebMessageFormat requestformat, out WebMessageFormat responseformat)
        {
